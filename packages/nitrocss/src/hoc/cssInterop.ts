...
import {
  withNitrowind,
  type NitrowindPropMapping,
  type WithNitrowindOptions,
  type WithNitrowindProps,
} from "./withNitrowind";
...
/** Normalize a shorthand mapping to `withNitrowind` options (exported for tests). */
export function normalizeCssInteropMapping<P>(
  mapping?: CssInteropMapping<P>,
): WithNitrowindOptions<P> | undefined {
  if (!mapping) return undefined;
  if (!isShorthandMapping(mapping)) return mapping;
  const props: Record<string, NitrowindPropMapping> = {};
  ...
  return props as WithNitrowindOptions<P>;
}
 ...
 * Thin ergonomic wrapper over {@link withNitrowind}.
 *
 * ```tsx
 ...
 *   {@link WithNitrowindOptions} form.
