 ...
 */
export const isTransformProp = (prop: string): boolean =>
  prop === "transform" ||
  prop === "translate" ||
  prop === "scale" ||
  prop === "rotate" ||
  prop.startsWith("--tw-translate-") ||
  prop.startsWith("--tw-scale-") ||
  prop.startsWith("--tw-skew-") ||
  prop.startsWith("--tw-rotate-");

const ANGLE_RE = /^-?\d*\.?\d+(deg|rad|grad|turn)$/;

/** Extract an angle from a bare value (`45deg`) or a function token (`skewX(3deg)`). */
const angle = (raw: string): string | undefined => {
  const v = raw.trim();
...
export function extractTransform(
  ...
  const lengthAxis = (prop: string, axis: TransformAxis): void => {
    const raw = get(prop);
  ...
  const scaleAxis = (prop: string, axis: TransformAxis): void => {
    const raw = get(prop);
    if (raw === undefined) return;
    const n = scale(raw);
  ...
  const angleAxis = (prop: string, axis: TransformAxis): void => {
    const raw = get(prop);
    if (raw === undefined) return;
    const a = angle(raw);
