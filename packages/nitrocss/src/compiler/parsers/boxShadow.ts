...
export function extractBoxShadow(
  ...
  return { boxShadow, ...extractNativeShadow(boxShadow), ...shadowColor };
}
...
function extractNativeShadow(boxShadow: string): RNStyle {
  ...
  const shadowColor = parsedColor
    ? formatHex({ ...parsedColor, alpha: 1 })
    : color;
