...
export const skillCatalog: SkillDefinition[] = [
  ...
  {
    id: "nitrowind-gradients",
    title: "Native Gradients",
    summary: "Build native linear, radial, and conic gradients with arbitrary values, runtime variables, positioned stops, and animated angles.",
    triggers: ["linear gradient", "radial gradient", "conic gradient", "animate gradient angle"],
    workflow: [
      "Choose `bg-linear-*`, `bg-radial-*`, or `bg-conic-*` from the geometry the design requires, then provide `from-*`, optional `via-*`, and `to-*` stops.",
      "Use arbitrary values or `bg-(image:--token)` only when a standard utility cannot express the gradient; keep runtime custom properties valid CSS image values.",
      "Animate the native gradient angle track for a moving conic sweep instead of rotating the entire view, its contents, or its border.",
      "Verify gradient rendering independently on iOS and Android because each platform uses its own native gradient layer.",
    ],
    example: `<View className="h-40 rounded-3xl bg-conic-45 from-rose-500 via-violet-500 to-cyan-400" />`,
    docs: [
      { label: "Gradients and Backgrounds", path: "/features/gradients-and-backgrounds" },
  ...
  {
    id: "nitrowind-grid",
    title: "Native Grid Layout",
    summary: "Create native grids with intrinsic tracks, auto and dense placement, named lines, spans, template areas, and masonry rows.",
    triggers: ["native grid", "grid auto placement", "named grid lines", "masonry layout", "min-content"],
    workflow: [
      "Put safe-area padding on a parent view and keep `grid` on its child so device insets do not alter track placement.",
      "Define explicit tracks first, then use auto rows or columns for implicit tracks and dense flow only when visual backfilling is acceptable.",
      "Use named lines or template areas for semantic placement, and use `min-content` or `max-content` when content should determine the intrinsic track size.",
      "Use masonry only for intentionally staggered content and verify item order remains understandable for accessibility.",
    ],
    docs: [
      { label: "Nitrowind-Specific Features", path: "/features/nitrowind-specific" },
      { label: "Responsive Layouts", path: "/features/responsive-and-containers" },
      { label: "Safe Area", path: "/features/safe-area" },
    ],
  },
  {
    id: "nitrowind-masks",
    title: "Native Masks and Clip Paths",
    summary: "Apply native image masks and clip paths on iOS and Android, including positioned masks, transparent fills, borders, and keyframe animation.",
    triggers: ["mask image", "native mask", "transparent image fill", "animate mask", "clip path"],
    workflow: [
      "Use `mask-image` for alpha-based image masking and `clip-path` for geometric clipping; do not treat them as interchangeable.",
      "Set mask repeat, position, and size explicitly when the asset should not tile or should remain centered.",
      "For a bordered photo mask with a transparent interior, compose the border and image layers while keeping the mask asset vector or high resolution to avoid pixelation.",
      "Animate the mask transform or opacity track with keyframes instead of rotating or pulsing the underlying image or its outer border.",
      "Verify the native result on both platforms and preserve a readable fallback when a mask asset cannot load.",
    ],
    docs: [
  ...
  {
    id: "nitrowind-accessibility",
    title: "Accessibility Variants",
    summary: "Adapt styles to reduced motion, increased contrast, reduced transparency, bold text, screen readers, and font scale.",
    triggers: ["reduced motion", "increased contrast", "screen reader styles", "font scale variant", "accessibility variant"],
    workflow: [
      "Start with an accessible base style, then use accessibility variants only for the visual or motion changes requested by the live native environment.",
      "Use `motion-reduce:`, `contrast-more:`, `reduce-transparency:`, `bold-text:`, and `screen-reader:` for boolean platform signals.",
      "Use `font-scale-[<condition>]:` when typography or layout must adapt at a measured native font scale.",
      "Test signal changes on a device or simulator and keep content order, labels, and touch targets accessible independently of styling.",
    ],
    docs: [
      { label: "Platforms", path: "/core-concepts/platforms" },
      { label: "Runtime State", path: "/core-concepts/runtime-state" },
