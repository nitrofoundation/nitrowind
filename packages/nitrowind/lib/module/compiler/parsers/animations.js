...
import { toRNProperty, toRNValue } from "../toRNValue.js";
...
export const isAnimationProp = prop => prop === "animation";

/** True for CSS transition declarations consumed by Reanimated's CSS engine. */
export const isTransitionProp = prop => prop === "transition-property" || prop === "transition-duration" || prop === "transition-delay" || prop === "transition-timing-function";

/** Collect a rule's `--reanimated-*` declarations into a plain object. */
export const extractReanimatedVars = declarations => {
  const vars = {};
...
export const normalizeTimingFunction = value => {
...
const transitionProperty = value => {
  ...
  const properties = splitCommaList(value).filter(prop => !prop.startsWith("--")).map(prop => toRNProperty(prop));
  return properties.length === 1 ? properties[0] : properties;
};

/** Coerce a CSS transition declaration into Reanimated's RN style props. */
export function foldTransition(prop, value, resolveVar) {
 ...
 */
export function parseTransformString(value, rem) {
  const out = [];
  let match;
 ...
 */
export function extractKeyframes(css, rem = 16) {
  const out = {};
 ...
 */
export function foldAnimation(shorthand, keyframes) {
  const props = {};
  let hasName = false;
