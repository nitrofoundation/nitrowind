"use strict";

import { formatHex, formatHex8, parse as parseColor } from "culori";
const VAR_RE = /var\(\s*(--[A-Za-z0-9-_]+)\s*(?:,\s*([^()]*))?\)/g;

/** Inline `var(--x[, fallback])` references using the supplied resolver. */
const resolveVars = (expr, resolveVar) => expr.replace(VAR_RE, (_, name, fallback) => {
  const v = resolveVar(name);
  return v ?? (fallback !== undefined ? fallback.trim() : "");
});

// Bare numbers not already carrying a unit and not part of a hex color.
const BARE_NUMBER = /(?<![#\w.-])[+-]?(?:\d*\.\d+|\d+)(?![\w-])/g;

/**
 * Build RN's `boxShadow` string from Tailwind's `--tw-shadow` helper (the
 * `box-shadow` longhand only composes ring/inset placeholders). Bare numbers
 * get an explicit `px` so RN's shadow string parser accepts them.
 */
export function extractBoxShadow(declarations, resolveVar) {
  const shadowColor = extractShadowColor(declarations, resolveVar);
  const raw = declarations.find(d => d.prop === "--tw-shadow")?.value;
  if (raw === undefined) return shadowColor;
  const resolved = resolveVars(raw, resolveVar).trim();
  // `0 0 #0000` is Tailwind's transparent placeholder (shadow-none / unset).
  if (!resolved || resolved === "0 0 #0000") return shadowColor;
  const boxShadow = resolved.replace(BARE_NUMBER, m => `${m}px`);
  return {
    boxShadow,
    ...extractNativeShadow(boxShadow),
    ...shadowColor
  };
}
function extractShadowColor(declarations, resolveVar) {
  const raw = declarations.find(d => d.prop === "--tw-shadow-color")?.value;
  if (raw === undefined || raw === "initial") return undefined;
  if (/\bcolor-mix\(/i.test(raw)) return undefined;
  if (raw.includes("var(")) {
    return {
      shadowColor: raw,
      shadowOpacity: 1,
      "--nitrowind-shadow-color": raw
    };
  }
  const resolved = resolveVars(raw, resolveVar).trim();
  const parsed = parseColor(resolved);
  if (!parsed) return undefined;
  const alpha = parsed.alpha ?? 1;
  return {
    shadowColor: formatHex({
      ...parsed,
      alpha: 1
    }),
    shadowOpacity: alpha,
    "--nitrowind-shadow-color": alpha < 1 ? formatHex8(parsed) : formatHex({
      ...parsed,
      alpha: 1
    })
  };
}
const LENGTH_TOKEN_RE = /^(-?\d*\.?\d+)px$/;
function extractNativeShadow(boxShadow) {
  const firstLayer = boxShadow.split(",", 1)[0]?.trim();
  if (!firstLayer || /^inset\b/i.test(firstLayer)) return {};
  const parts = firstLayer.split(/\s+/).filter(Boolean);
  const lengths = [];
  let color;
  for (const part of parts) {
    const length = LENGTH_TOKEN_RE.exec(part);
    if (length) {
      lengths.push(Number(length[1]));
      continue;
    }
    if (!color && parseColor(part)) color = part;
  }
  if (lengths.length < 2) return {};
  const parsedColor = color ? parseColor(color) : undefined;
  const opacity = parsedColor?.alpha ?? 1;
  const shadowColor = parsedColor ? formatHex({
    ...parsedColor,
    alpha: 1
  }) : color;
  const offsetX = lengths[0] ?? 0;
  const offsetY = lengths[1] ?? 0;
  const blur = Math.max(0, lengths[2] ?? 0);
  return {
    ...(shadowColor ? {
      shadowColor
    } : {}),
    shadowOffset: {
      width: offsetX,
      height: offsetY
    },
    shadowOpacity: opacity,
    shadowRadius: blur,
    elevation: Math.max(1, Math.round(Math.max(Math.abs(offsetY), blur) / 2))
  };
}

/** True for declarations consumed by the box-shadow parser. */
export const isBoxShadowProp = prop => prop === "box-shadow" || prop === "-webkit-box-shadow";
//# sourceMappingURL=boxShadow.js.map