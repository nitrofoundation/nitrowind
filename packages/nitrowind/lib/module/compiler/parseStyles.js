...
import { dependencyFromAtRule, dependencyFromSelector, dependencyFromValue, flag, union } from "./dependencies.js";
import { parseInsetValue } from "./insetValue.js";
import { CONTAINER_DECL_PROPS, containerMarkerFromDeclarations, isCustomContainerToken, parseContainerQuery } from "./container.js";
import { extractBoxShadow, extractFilter, extractFontVariant, extractKeyframes, extractReanimatedVars, extractTextShadow, extractTransform, foldAnimation, foldTransition, isAnimationProp, isBoxShadowProp, isFilterProp, isFontVariantProp, isTextShadowProp, isTransitionProp, isTransformProp } from "./parsers/index.js";
import { platformFromSelector } from "./platform.js";
import { toRNProperties, toRNValue } from "./toRNValue.js";
 ...
 */
export function classTokenFromSelector(selector) {
 ...
 */
const isParsedProp = prop => prop.startsWith("--") || isTransformProp(prop) || isBoxShadowProp(prop) || isFilterProp(prop) || isTextShadowProp(prop) || isFontVariantProp(prop) || isAnimationProp(prop) || isTransitionProp(prop) || CONTAINER_DECL_PROPS.has(prop) || prop === "backdrop-filter" || prop === "-webkit-backdrop-filter";
 ...
 */
export function parseStyles(css, rem, resolveVar = defaultResolveVar) {
  const classes = {};
  ...
  for (const rule of walkRules(css)) {
    ...
    const style = {};
    let mask = union(...rule.atRules.map(dependencyFromAtRule), dependencyFromSelector(rule.selector));
    ...
    const animationDecl = rule.declarations.find(d => isAnimationProp(d.prop));
    if (animationDecl) {
      const folded = foldAnimation(animationDecl.value, keyframes);
      if (folded) Object.assign(style, folded);
    }
    for (const transitionDecl of rule.declarations.filter(d => isTransitionProp(d.prop))) {
      const folded = foldTransition(transitionDecl.prop, transitionDecl.value, ruleResolve);
      if (folded) Object.assign(style, folded);
    }
    ...
    const container = containerPrelude ? parseContainerQuery(containerPrelude, rem, ruleResolve) : undefined;
