...
import { flag } from "./dependencies.js";
...
export const CONTAINER_DECL_PROPS = new Set(["container", "container-type", "container-name"]);
 ...
 */
export function parseContainerQuery(prelude, rem, resolveVar = noResolve) {
 ...
 */
export function containerMarkerFromDeclarations(declarations) {
  let type;
  let name;
  for (const {
    prop,
 ...
 */
export function parseCustomContainerToken(token, rem, resolveVar = noResolve) {
...
export const isCustomContainerToken = token => CUSTOM_TOKEN_RE.test(token);
 ...
 */
export function applyCustomContainerTokens(artifact, tokens, rem, resolveVar = noResolve) {
