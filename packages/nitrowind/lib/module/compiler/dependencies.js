...
export const flag = dep => 1 << dep;
export const addFlag = (mask, dep) => mask | flag(dep);
export const hasFlag = (mask, dep) => (mask & flag(dep)) !== 0;
export const union = (...masks) => masks.reduce((acc, m) => acc | m, 0);

/** Expand a mask into the list of `StyleDependency` values it contains. */
export const toList = mask => {
  const out = [];
  for (const dep of [StyleDependency.Theme, StyleDependency.ColorScheme, StyleDependency.Dimensions, StyleDependency.Insets, StyleDependency.Orientation, StyleDependency.Rtl, StyleDependency.FontScale, StyleDependency.Rem, StyleDependency.ContainerSize, StyleDependency.GroupState]) {
 ...
 */
export const dependencyFromAtRule = condition => {
  const c = condition.toLowerCase();
  let mask = 0;
  if (c.includes("prefers-color-scheme")) mask = addFlag(mask, StyleDependency.ColorScheme);
  // A `@container` query reads the nearest container's size, not the screen.
  if (c.startsWith("@container")) return addFlag(mask, StyleDependency.ContainerSize);
  if (c.includes("width") || c.includes("height")) mask = addFlag(mask, StyleDependency.Dimensions);
  if (c.includes("orientation")) mask = addFlag(mask, StyleDependency.Orientation);
  if (c.includes("resolution") || c.includes("dpi")) mask = addFlag(mask, StyleDependency.Dimensions);
  return mask;
};
 ...
 */
export const dependencyFromSelector = selector => {
...
export const dependencyFromValue = value => {
  let mask = 0;
  ...
  if (value.includes("env(safe-area-inset")) mask = addFlag(mask, StyleDependency.Insets);
  if (/\d*\.?\d+rem\b/.test(value)) mask = addFlag(mask, StyleDependency.Rem);
  if (/\d*\.?\d+em\b/.test(value)) mask = addFlag(mask, StyleDependency.FontScale);
  return mask;
};
