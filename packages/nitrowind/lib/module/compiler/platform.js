...
export const PLATFORMS = ["ios", "android", "web", "native", "macos", "windows"];
 ...
 */
export const PLATFORM_CSS = PLATFORMS.map(platform => `@custom-variant ${platform} (&:where([${PLATFORM_MARKER}="${platform}"], [${PLATFORM_MARKER}="${platform}"] *));`).join("\n");
 ...
 */
export function platformFromSelector(selector) {
