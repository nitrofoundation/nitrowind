...
import { walkRules } from "./parseStyles.js";
import { normalizeTimingFunction } from "./parsers/animations.js";
import { normalizeColorValue } from "./toRNValue.js";
const THEME_BLOCK_RE = /@theme[^{]*\{([\s\S]*?)\}/g;
const VAR_RE = /(--[A-Za-z0-9-_]+)\s*:\s*([^;]+);/g;
const normalizeThemeValue = (name, value) => {
  const raw = value.trim();
  return name.startsWith("--ease-") || name === "--default-transition-timing-function" ? normalizeTimingFunction(raw) : normalizeColorValue(raw);
};
 ...
 */
export function extractThemes(css, baseThemeName = "light") {
  const themes = {};
  const order = [];
  const ensure = name => {
    if (!themes[name]) {
