...
import { toRNValue } from "../compiler/toRNValue.js";
import { isInsetValue } from "../compiler/types.js";
import { ColorScheme } from "../specs/types.js";
import { toList } from "./mask.js";
import { foldTransform, normalizeShadow } from "./normalize.js";
import { buildEnteringAnimation, buildExitingAnimation, buildLayoutAnimation, hasReanimatedVars } from "./reanimated.js";
import { getArtifact, getArtifactVersion, getClassBuckets } from "./registry.js";
...
const isUnsupportedNativeColorValue = (prop, value) => Platform.OS !== "web" && NATIVE_COLOR_PROPS.has(prop) && /^color-mix\(/i.test(value.trim());
...
function snapshotKey(snapshot) {
  return [getArtifactVersion(), Platform.OS, snapshot.currentThemeName, snapshot.colorScheme, snapshot.orientation, snapshot.screen.width, snapshot.screen.height, snapshot.insets.top, snapshot.insets.right, snapshot.insets.bottom, snapshot.insets.left, snapshot.pixelRatio, snapshot.fontScale, snapshot.rtl ? 1 : 0, snapshot.rem, snapshot.hairlineWidth].join("|");
}
...
function effectiveVars(snapshot) {
  ...
  const scheme = snapshot.colorScheme === ColorScheme.Dark ? artifact.themes["dark"] : artifact.themes["light"];
  return scheme ? {
    ...base,
    ...scheme
  } : base;
  ...
  const applyBucketStyle = (target, bucketStyle) => {
    for (const [prop, value] of Object.entries(bucketStyle)) {
      ...
      if (typeof value === "string" && value.includes("var(")) {
        ...
        target[prop] = toRNValue(prop, resolved, {
          rem
        }) ?? resolved;
      } else {
...
export function resolveStyles(className, snapshot, state) {
