...
try {
  const sibling = path.join(path.dirname(upstreamPath), "metro-transform-worker.js");
...
const inputAbs = process.env.NITROWIND_INPUT ? path.resolve(process.env.NITROWIND_INPUT) : null;
...
function isStylesheet(projectRoot, filename) {
  ...
  const abs = path.isAbsolute(filename) ? filename : path.resolve(projectRoot, filename);
