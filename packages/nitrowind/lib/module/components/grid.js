...
function horizontalPadding(style) {
  const flattened = typeof StyleSheet.flatten === "function" ? StyleSheet.flatten(style) : Array.isArray(style) ? Object.assign({}, ...style.filter(Boolean)) : style;
...
export function withGridFallback(children, parentClassName, containerWidth = 0) {
...
export function useGridFallback(children, parentClassName, onLayout, parentStyle) {
  ...
  const nextChildren = useMemo(() => withGridFallback(children, parentClassName, containerWidth), [children, parentClassName, containerWidth]);
  return {
