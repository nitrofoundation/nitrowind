...
function registerContainer(id, name) {
...
function setContainerSize(id, size) {
  const entry = byId.get(id);
  if (!entry) return;
...
function getNamedContainerSize(name) {
  const id = nameToId.get(name);
  return id ? getContainerSize(id) : undefined;
}
function subscribeContainer(id, listener) {
  const entry = byId.get(id);
  if (!entry) return () => {};
...
function subscribeNamedContainer(name, listener) {
 ...
 */
function resolveContainerId(condition, nearestId, named) {
...
function matchesCondition(condition, size) {
 ...
 */
function evaluateContainerQueries(queries, sizeFor) {
  const merged = {};
