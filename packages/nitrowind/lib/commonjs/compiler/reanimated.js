...
exports.REANIMATED_CSS = exports.LAYOUT_PRESETS = exports.ENTERING_EXITING_PRESETS = exports.CSS_ANIMATIONS = void 0;
/**
...
const PRESET_TYPES = ["entering", "exiting", "layout"];
const TIME_TYPES = ["duration", "delay"];
const DEFAULT_TIME_VALUES = [75, 100, 150, 200, 300, 500, 700, 1000];
const SPRING_TYPES = ["damping", "stiffness", "mass"];
const EASINGS = [["linear", "linear"], ["in", "ease-in"], ["out", "ease-out"], ["in-out", "ease-in-out"], ["bounce", "ease-bounce"]];
const utility = (name, body) => `@utility ${name} {\n  ${body}\n}\n`;

/** Build the entering/exiting/layout preset utilities. */
function buildPresetUtilities() {
  let css = "";
...
function buildConfigUtilities() {
  let css = "";
  for (const type of PRESET_TYPES) {
    for (const timeType of TIME_TYPES) {
      for (const value of DEFAULT_TIME_VALUES) {
        css += utility(`${type}-${timeType}-${value}`, `--reanimated-${type}-${timeType}: ${value}ms;`);
      }
      css += utility(`${type}-${timeType}-*`, `--reanimated-${type}-${timeType}: --value(integer)ms;`);
    }
    for (const physics of SPRING_TYPES) {
      css += utility(`${type}-${physics}-*`, `--reanimated-${type}-${physics}: --value(number);`);
    }
    for (const [suffix, value] of EASINGS) {
      css += utility(`${type}-ease-${suffix}`, `--reanimated-${type}-easing: ${value};`);
    }
    css += utility(`${type}-springify`, `--reanimated-${type}-springify: true;`);
  }
  return css;
}

/** Build the `@keyframes` + `animate-*` CSS-animation utilities. */
function buildCssAnimations() {
  let css = "";
  for (const anim of CSS_ANIMATIONS) {
    const steps = Object.entries(anim.keyframes).map(([selector, props]) => {
 ...
 */
const REANIMATED_CSS = exports.REANIMATED_CSS = [buildPresetUtilities(), buildConfigUtilities(), buildCssAnimations()].join("\n");
