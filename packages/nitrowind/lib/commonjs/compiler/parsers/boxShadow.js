...
exports.extractBoxShadow = extractBoxShadow;
exports.isBoxShadowProp = void 0;
 ...
 */
function extractBoxShadow(declarations, resolveVar) {
  ...
  return {
    boxShadow,
    ...extractNativeShadow(boxShadow),
    ...shadowColor
  };
}
function extractShadowColor(declarations, resolveVar) {
  ...
  const resolved = resolveVars(raw, resolveVar).trim();
  const parsed = (0, _culori.parse)(resolved);
  if (!parsed) return undefined;
  const alpha = parsed.alpha ?? 1;
  return {
    shadowColor: (0, _culori.formatHex)({
      ...parsed,
      alpha: 1
    }),
    shadowOpacity: alpha,
    "--nitrowind-shadow-color": alpha < 1 ? (0, _culori.formatHex8)(parsed) : (0, _culori.formatHex)({
      ...parsed,
      alpha: 1
    })
  };
}
const LENGTH_TOKEN_RE = /^(-?\d*\.?\d+)px$/;
function extractNativeShadow(boxShadow) {
  const firstLayer = boxShadow.split(",", 1)[0]?.trim();
  if (!firstLayer || /^inset\b/i.test(firstLayer)) return {};
  const parts = firstLayer.split(/\s+/).filter(Boolean);
  const lengths = [];
  let color;
  for (const part of parts) {
    const length = LENGTH_TOKEN_RE.exec(part);
    if (length) {
      lengths.push(Number(length[1]));
      continue;
    }
    if (!color && (0, _culori.parse)(part)) color = part;
  }
  if (lengths.length < 2) return {};
  const parsedColor = color ? (0, _culori.parse)(color) : undefined;
  const opacity = parsedColor?.alpha ?? 1;
  const shadowColor = parsedColor ? (0, _culori.formatHex)({
    ...parsedColor,
    alpha: 1
  }) : color;
  ...
  return {
    ...
    shadowRadius: blur,
    elevation: Math.max(1, Math.round(Math.max(Math.abs(offsetY), blur) / 2))
  };
}

/** True for declarations consumed by the box-shadow parser. */
const isBoxShadowProp = prop => prop === "box-shadow" || prop === "-webkit-box-shadow";
exports.isBoxShadowProp = isBoxShadowProp;
