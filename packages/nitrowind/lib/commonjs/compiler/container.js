...
exports.CONTAINER_DECL_PROPS = void 0;
exports.applyCustomContainerTokens = applyCustomContainerTokens;
exports.containerMarkerFromDeclarations = containerMarkerFromDeclarations;
exports.isCustomContainerToken = void 0;
exports.parseContainerQuery = parseContainerQuery;
exports.parseCustomContainerToken = parseCustomContainerToken;
...
const CONTAINER_DECL_PROPS = exports.CONTAINER_DECL_PROPS = new Set(["container", "container-type", "container-name"]);
const noResolve = () => undefined;
const RANGE_RE = /(width|height|inline-size|block-size)\s*(>=|<=|>|<)\s*(-?[\d.]+(?:px|rem|em)?)/i;
const MINMAX_RE = /(min|max)-(width|height|inline-size|block-size)\s*:\s*(-?[\d.]+(?:px|rem|em)?)/i;
const toAxis = raw => raw === "height" || raw === "block-size" ? "height" : "width";
 ...
 */
function parseContainerQuery(prelude, rem, resolveVar = noResolve) {
  ...
  if (range) {
    const value = (0, _insetValue.lengthToPx)(range[3], resolveVar, rem);
  ...
  if (minmax) {
    const value = (0, _insetValue.lengthToPx)(minmax[3], resolveVar, rem);
    if (value === undefined) return undefined;
    const op = minmax[1].toLowerCase() === "min" ? ">=" : "<=";
    return {
      ...(name ? {
        name
      } : {}),
 ...
 */
function containerMarkerFromDeclarations(declarations) {
  let type;
  let name;
  for (const {
    prop,
    ...
    if (prop === "container-type") {
      ...
      if (namePart && namePart !== "none") {
        if (typePart) name = namePart;else if (namePart === "inline-size" || namePart === "size") type = namePart;else name = namePart;
      }
      type ??= "inline-size";
    }
  }
  ...
  return {
    ...
    type: type ?? "inline-size"
  };
}
const CUSTOM_TOKEN_RE = /^\[(?:parent|cq)-([wh])(>=|<=|>|<)(-?[\d.]+(?:px|rem|em)?)\](?:\/([a-zA-Z][\w-]*))?:(.+)$/;
 ...
 */
function parseCustomContainerToken(token, rem, resolveVar = noResolve) {
  const m = CUSTOM_TOKEN_RE.exec(token);
  if (!m) return undefined;
  const value = (0, _insetValue.lengthToPx)(m[3], resolveVar, rem);
  if (value === undefined) return undefined;
  const name = m[4];
  return {
    token,
...
const isCustomContainerToken = token => CUSTOM_TOKEN_RE.test(token);
 ...
 */
exports.isCustomContainerToken = isCustomContainerToken;
function applyCustomContainerTokens(artifact, tokens, rem, resolveVar = noResolve) {
  for (const token of tokens) {
    ...
    const base = baseBuckets.find(b => b.variant === "base" && !b.platform && !b.container && !b.containerMarker) ?? baseBuckets[0];
    const bucket = {
      style: {
        ...base.style
      },
      dependencies: base.dependencies | (0, _dependencies.flag)(_types.StyleDependency.ContainerSize),
      variant: "base",
