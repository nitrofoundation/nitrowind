...
void StyleEngine::setCompiledStyles(const std::string& json) {
...
void StyleEngine::registerThemes(const std::vector<std::string>& names) {
  std::lock_guard<std::mutex> lock(mutex_);
  themeNames_ = names;
  if (currentTheme_.empty() && !themeNames_.empty()) {
    currentTheme_ = themeNames_.front();
...
void StyleEngine::setTheme(const std::string& name) {
  std::lock_guard<std::mutex> lock(mutex_);
  currentTheme_ = name;
}

std::string StyleEngine::currentTheme() const {
  std::lock_guard<std::mutex> lock(mutex_);
  return currentTheme_;
}

bool StyleEngine::hasTheme(const std::string& name) const {
  std::lock_guard<std::mutex> lock(mutex_);
  return themes_.find(name) != themes_.end();
}

double StyleEngine::rem() const {
  std::lock_guard<std::mutex> lock(mutex_);
  return rem_;
}

uint32_t StyleEngine::dependencyMask(const std::string& className) const {
  std::lock_guard<std::mutex> lock(mutex_);
  uint32_t mask = 0;
...
bool StyleEngine::resolveContainerMarker(const std::string& className,
...
bool StyleEngine::platformApplies(const std::string& platform) {
...
bool StyleEngine::containerMatches(const ContainerCondition& condition,
                                   const ResolveContext& ctx) {
...
folly::dynamic StyleEngine::effectiveVars(const ResolveContext& ctx) const {
...
folly::dynamic StyleEngine::resolve(const std::string& className,
                                    const ResolveContext& ctx,
