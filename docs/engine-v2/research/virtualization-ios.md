...
- **`ShadowTreeMutator`** — `packages/nitrowind/cpp/fabric/ShadowTreeMutator.hpp`:
  ...
  node.
- **`LayoutObserver`** — `packages/nitrowind/cpp/fabric/LayoutObserver.{hpp,cpp}`:
  ...
  **This is our geometry source** — and, unlike RN, it's shadow-tree geometry, not
  UIKit `convertRect:`.
- **`LinkedNode`** — `packages/nitrowind/cpp/registry/LinkedNode.hpp`: keyed by
...
- `packages/nitrowind/cpp/fabric/ShadowTreeMutator.hpp` — `commit(vector<NodeMutation{Tag,folly::dynamic props}>)`.
- `packages/nitrowind/cpp/fabric/LayoutObserver.{hpp,cpp}` — `shadowTreeDidMount` walk reading `getLayoutMetrics().frame`.
- `packages/nitrowind/cpp/registry/LinkedNode.hpp` — `Tag` → `ShadowNodeFamily::Shared` per-item handle.
