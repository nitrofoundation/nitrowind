...
Semantic colors keep the operating system token in the compiled style instead of replacing it with a fixed hex value. iOS can resolve UIKit semantic colors and Android can resolve theme attributes at paint time, including system appearance and accessibility changes.

The native engine sends React Native's native color objects into Fabric:

- iOS platform tokens remain `PlatformColor` semantic names.
- Android tokens remain `PlatformColor` resource paths/theme attributes.
- Dynamic pairs remain `DynamicColorIOS`, including high-contrast light and dark branches.
...
The semantic token layer defines cross-platform aliases for `label`, `secondaryLabel`, `systemBackground`, `secondarySystemBackground`, `separator`, `link`, and `accent`. The iOS adapter maps them to UIKit colors; the Android adapter maps them to theme attributes.
