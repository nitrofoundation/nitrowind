  ...
  return (
    <Screen>
      ...
      <Section
        ...
        subtitle="Pure CSS @keyframes run in the browser or Reanimated's native CSS engine — no JS frame driver."
      >
        <View className="flex-row flex-wrap gap-3">
          {LOOPS.map(a => (
...
const ClassTransition = () => {
  ...
  return (
    <Section
      ...
      subtitle="transition-all, transition-colors, duration and easing work through browser CSS and native Reanimated."
    >
      <Pressable
...
const EnterExit = () => {
  ...
  return (
    ...
    >
      <PillButton label={showExit ? 'Hide' : 'Show'} onPress={toggle} />
      {showExit ? (
        <View
          className={`h-20 items-center justify-center rounded-2xl bg-rose-500 entering-duration-300 ${
            leaving
              ? 'web-exiting-fade-out-down'
              : 'entering-fade-in-up exiting-fade-out-down'
          }`}
        >
